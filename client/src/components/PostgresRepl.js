...
export default class PostgresRepl extends Component {
    state = {
        res: {
            startTime: 0,
            endTime: 0,
    ...
    onClickInsert = async () => {
        ...
        await axios.post("http://localhost:5000/api/todos/pg-insert").then((res) => this.setState({ res: res.data })).then(() => this.setState({ ...this.state, isFinished: true })).catch((err) => console.log(err));

    }
    onClickSelect = async () => {
        await this.setState({ ...this.state, isFinished: false });
        await axios.get("http://localhost:5000/api/todos/pg-select").then((res) => this.setState({ res: res.data })).then(() => this.setState({ ...this.state, isFinished: true })).catch((err) => console.log(err));
    }
        ...
        return (
            <div>
                <Col md={'12'}>
                    <Row>
                        <Col className="mt-1">
                            <Alert.Heading>Postgres Replication</Alert.Heading>
                            <p>
